...
This algorithm generates a perfect binary tree of depth $n$. We then present the following algorithm to generate all the leaves of the perfect binary tree:

\begin{algorithm}[hbt!]
  \caption{Perfect Binary Tree Leaves Generator}\label{alg:leaves-generator}
  \KwData{$n \geq 0$}
  \KwResult{A perfect binary tree graph's leaves' labels}
  ...
  \SetKwProg{Fn}{Function}{:}{}
  \Fn{\FMain{$n$}}{
    $num\_vertices \gets 2^{n - 1}$\;
    $leaves \gets []$\;
    $last\_row\_start \gets int(log(n - 1, 2)) - 1$\;
    \For{$vertex$ in range($last\_row\_start$, $num\_vertices$)}{
      $leaves$.append($vertex + num\_vertices - 1$)\;
    }
    \Return{$leaves$}
  }
\end{algorithm}
